...
class TensorflowNN(tf.keras.Model):
    def __init__(self):
        ...
        self.fc4 = layers.Dense(2, activation='softmax', kernel_initializer=initializers.GlorotUniform())
...
def pytorch_network(x, y):
    ...
    parameters = load_parameters("./parameters/initial_binary_softmax")
...
def my_mlp_network(x, y):
    # my network
    topology = load_topology("./topologies/binary_softmax.json")
    parameters = load_parameters("./parameters/initial_binary_softmax")
